...
public class Main {
	...
	static void Program78() {
		NoticeboardInterface ui = new NoticeboardInterface();
        SwingUtilities.invokeLater(ui);
	}
	
	static void Program79() {
		clicks.Calculator calculator = new clicks.Calculator();
		clicks.UserInterface ui = new clicks.UserInterface(calculator);
        SwingUtilities.invokeLater(ui);
	}
	
	static void Program80() {
		calculator.Calculator calculator = new calculator.Calculator();
		calculator.UserInterface ui = new calculator.UserInterface(calculator);
		SwingUtilities.invokeLater(ui);
	}
	
	static void Program81() {
		drawing.UserInterface ui = new drawing.UserInterface(new drawing.Avatar(30, 30));
        SwingUtilities.invokeLater(ui);
	}
	
	static void Program82() {
		movingFigure.CompoundFigure truck = new movingFigure.CompoundFigure();

        truck.add(new movingFigure.Box(220, 110, 75, 100));
        truck.add(new movingFigure.Box(80, 120, 200, 100));
        truck.add(new movingFigure.Circle(100, 200, 50));
        truck.add(new movingFigure.Circle(220, 200, 50));

        movingFigure.UserInterface ui = new movingFigure.UserInterface(truck);
        SwingUtilities.invokeLater(ui);
	}
	
	static void Program83() {
		advancedCalculator.UserInterface ui = new advancedCalculator.UserInterface();
		SwingUtilities.invokeLater(ui);
	}
	
	static void Program90() {
		wormgame.WormGame wormGame = new wormgame.WormGame();
		wormgame.UserInterface ui = new wormgame.UserInterface(wormGame);
		SwingUtilities.invokeLater(ui);
